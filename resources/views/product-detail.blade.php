...
<div class="bg-white flex flex-col lg:flex-row text-ah-medium-dark-grey" id="advantage">
    <div class="lg:w-1/2 py-[70px] px-[20px] lg:py-[63px] lg:pl-[163px] lg:pr-[97px]">
        <h2 class="text-[43px] lg:text-[46px] font-black leading-[120%] mb-[50px] uppercase">The {{$product['name']}}<br> advantage:</h2>
        {!! $product['below_fold'] !!}
    </div>
    ...
    <div class="lg:w-1/2 py-[70px] px-[20px] lg:pl-[80px] lg:pr-[195px] text-ah-medium-dark-grey">
        ...
        @foreach ($product['files'] as $file )
            <a href="{{asset($file['link'])}}" download="{{$file['name']}}" class="flex justify-between h-[70px] px-5 items-center text-ah-medium-dark-grey border-l-[7px] border-l-ah-red align-middle cursor-pointer">
                <img src="{{asset('images/general/Sell-Sheet.svg')}}" class="w-[32px] h-[32px]" alt="">
                <span class="text-[18px] font-semibold leading-[150%] mr-[100px]">Sell Sheet</span>
                <img src="{{asset('images/general/arrow-right.svg')}}">
            </a>
        @endforeach
        </div>
    </div>
